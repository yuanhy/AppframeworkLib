...
//				}
				...
				break;
			case MotionEvent.ACTION_UP:
				Log.v(getClass().getName(),"onTouchEvent--> MotionEvent.ACTION_UP:");
			final 	VelocityTracker velocityTracker= mVelocityTracker;
				velocityTracker.computeCurrentVelocity(1000,mMaximumVelocity);
				int initialVeLocity= (int) velocityTracker.getYVelocity();
				if(	scroller.springBack(getScrollX(),getScrollY(),0,0,0,distanceY)) {
					postInvalidateOnAnimation();
					Log.v(getClass().getName(),"onTouchEvent--> MotionEvent.ACTION_UP:springBack");
				}else	if (Math.abs(initialVeLocity)>mMinimumVelocity){
					fling(-initialVeLocity);
					Log.v(getClass().getName(),"onTouchEvent--> MotionEvent.ACTION_UP:fling");
				}
				break;
		}
		return true;
