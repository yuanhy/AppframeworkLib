package com.yuanhy.library_tools.callback;
...
public class SwipeCardCallback<T> extends ItemTouchHelper.SimpleCallback {
    ...
    private UniversalAdapter<T> adapter;
    private List<T> mDatas;

    public SwipeCardCallback(RecyclerView mRv,
                             UniversalAdapter<T> adapter, List<T> mDatas) {
        //方向
        super(0, 15);//1111
        this.mRv = mRv;
    ...
    public void onSwiped(@NonNull RecyclerView.ViewHolder viewHolder, int i) {
        T remove = mDatas.remove(viewHolder.getLayoutPosition());//第八个
