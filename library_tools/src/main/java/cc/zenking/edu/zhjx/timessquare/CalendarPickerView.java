...
public class CalendarPickerView extends ListView {
    ...
    private Calendar currentNetCalendar = null;
    ...
    private Map<Integer, List<Date>> dates_private = null;
    ...
    private boolean singleCancel = false;
    ...
    public void setPrivateDates(Map<Integer, List<Date>> dates) {
        dates_private = dates;
    }
    ...
    public CalendarPickerView(Context context, AttributeSet attrs) {
        ...
        dividerColor = a.getColor(
                ...
                res.getColor(R.color.color_ffffff));
        ...
        if (currentNetCalendar == null) {
            today = Calendar.getInstance(locale);
        } else {
            today = currentNetCalendar;
        }
        ...
        if (isInEditMode()) {
            ...
            init(new Date(), nextYear.getTime(), minNum, maxNum) //
     ...
     *                {@code minDate}.
     */
    public FluentInitializer init(Date minDate, Date maxDate, int minNum, int maxNum) {
        this.minNum = minNum;
        this.maxNum = maxNum;
        commonPosition = -minNum;
        return init(minDate, maxDate, Locale.getDefault());
    }

    public void setCurrentNetCalendar(Calendar currentNetCalendar) {
        this.currentNetCalendar = currentNetCalendar;
    }
    ...
    public boolean scrollToDate(Date date) {
        ...
        if (selectedIndex != null) {
            if (ApplyForLeave.PAGENAME.equals(pageName)) {
                commonPosition = selectedIndex;
                adapter.notifyDataSetChanged();
            } else {
                scrollToSelectedMonth(selectedIndex);
            }
    ...
    public void setSingleCanCancel(boolean singleCanCancel) {
        this.singleCanCancel = singleCanCancel;
    }
    ...
    public boolean selectDate(Date date, boolean smoothScroll) {
        boolean isRun = validateDate(date);
        if (isRun) {
            MonthCellWithMonthIndex monthCellWithMonthIndex = getMonthCellWithIndexByDate(date);
            if (monthCellWithMonthIndex == null || !isDateSelectable(date)) {
                return false;
            }
            boolean wasSelected = doSelectDate(date,
                    monthCellWithMonthIndex.cell);
            if (wasSelected) {
    ...
    private boolean doSelectDate(Date date, MonthCellDescriptor cell) {
        ...
        switch (selectionMode) {
            ...
            case SINGLE:
                clearOldSelections(date, newlySelectedCal);
                break;
            default:
    ...
    private void clearOldSelections(Date date, Calendar selectedCal) {
        for (MonthCellDescriptor selectedCell : selectedCells) {
            // De-select the currently-selected cell.
            selectedCell.setSelected(false);
        ...
        if (selectedCells != null && selectedCells.size() != 0 && selectedCals != null
                && selectedCals.size() != 0 && selectedCells.get(0).getDate().equals(date)
                && sameDate(selectedCals.get(0), selectedCal) && singleCanCancel) {
            singleCancel = true;
        } else {
            singleCancel = false;
        }
    ...
    private class MonthAdapter extends BaseAdapter {
        ...
        @Override
        public int getCount() {
            return ApplyForLeave.PAGENAME.equals(pageName) ? 1 : months
                    .size();
        }

        @Override
        public Object getItem(int position) {
            return ApplyForLeave.PAGENAME.equals(pageName) ? months
                    .get(1) : months.get(position);
        }

        @Override
        public long getItemId(int position) {
            return ApplyForLeave.PAGENAME.equals(pageName) ? 1 : position;
        }
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (monthView == null
                    || !monthView.getTag(R.id.monthview_tag_key).equals(
                    dayViewAdapter.getClass())) {
                monthView = MonthView.create(parent, inflater,
                        ...
                        dayViewAdapter);
                monthView.setTag(R.id.monthview_tag_key,
                        dayViewAdapter.getClass());
//                AutoUtils.autoSize(monthView);
            } else {
                monthView.setDecorators(decorators);
            }
            // TODO 根据pageName判断如果是ApplyForLeave.PAGENAME则显示特定的view
            TextView title = (TextView) monthView.findViewById(R.id.title);
            RelativeLayout rl_title_view = (RelativeLayout) monthView
                    .findViewById(R.id.rl_title_view);
            RelativeLayout rl_title = (RelativeLayout) monthView
                    .findViewById(R.id.rl_title);
            if (ApplyForLeave.PAGENAME.equals(pageName)) {
                rl_title_view.setVisibility(View.VISIBLE);
                rl_title.setVisibility(View.GONE);
                ImageView leftarrow = (ImageView) monthView
                        .findViewById(R.id.iv_left_arrow);
                ImageView rightarrow = (ImageView) monthView
                ...
                leftarrow.setOnClickListener(new OnClickListener() {
                    ...
                    public void onClick(View v) {
                        if (commonPosition > 0) {
                            ...
                            Toast.makeText(context, "不能往前翻页了~",
                ...
                rightarrow.setOnClickListener(new OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        if (commonPosition < maxNum - minNum) {
                            commonPosition++;
                            adapter.notifyDataSetChanged();
                        } else {
                            Toast.makeText(context, "不能往后翻页了~",
                                    Toast.LENGTH_SHORT).show();
                        }
                    }
                });
            ...
            monthView.init(months.get(position), cells.get(position),
                    displayOnly, titleTypeface, dateTypeface, dates_private,
                    onlyShow);
            return monthView;
        }
    }
