...
public class CalendarRowView extends ViewGroup implements View.OnClickListener {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		for (int c = 0, numChildren = getChildCount(); c < numChildren; c++) {
			...
			cellSize = r - l;
			int cellWidthSpec = makeMeasureSpec(6 * cellSize / 7, EXACTLY);
			int cellHeightSpec = isHeaderRow ? makeMeasureSpec(
					6 * cellSize / 7, AT_MOST) : cellWidthSpec;
		...
		final int widthWithPadding = totalWidth + getPaddingLeft()
				+ getPaddingRight();
		final int heightWithPadding = 7 * rowHeight / 6 + getPaddingTop()
				+ getPaddingBottom();
	...
	protected void onLayout(boolean changed, int left, int top, int right,
		...
		for (int c = 0, numChildren = getChildCount(); c < numChildren; c++) {
			...
			if (isHeaderRow) {
				int l = ((c + 0) * width) / 7;
				int r = ((c + 1) * width) / 7;
				child.layout(l, 0, r, cellSize);
			} else {
				int l = (c + 0) * cellSize;
				int r = (c + 1) * cellSize;
				child.layout(l + margin, 0 + margin, r - margin, cellSize
						- margin);
			}
		}
